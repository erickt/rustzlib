...
fn compress(src: ~[u8], level: int) -> ~[u8] unsafe {
    ...
    let destlen = libz::compressBound(srclen);
    let dest = vec::to_mut(vec::from_elem::<u8>(destlen as uint, 0u8));
    let pdest = vec::unsafe::to_ptr::<u8>(dest);
    let psrc = vec::unsafe::to_ptr::<u8>(src);
    let pdestlen = ptr::addr_of::<u64>(destlen);
    let r = libz::compress2(pdest, pdestlen, psrc, srclen, level as c_int);
    // XXX: 0 == Z_OK
    if r != 0 as c_int {
...
fn uncompress(src: ~[u8], _destlen: u64) -> ~[u8] unsafe {
    ...
    let pdest = vec::unsafe::to_ptr::<u8>(dest);
    let psrc = vec::unsafe::to_ptr::<u8>(src);
    let r = libz::uncompress(pdest, pdestlen, psrc, srclen);
    if r != 0 as c_int {
